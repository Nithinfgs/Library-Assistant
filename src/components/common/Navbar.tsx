...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const collections: { id: CollectionType; name: string; icon: React.ReactNode }[] = [
    { id: 'academic', name: 'Academic', icon: <GraduationCap className="w-3.5 h-3.5 text-[#4338CA]" /> },
    { id: 'fiction', name: 'Fiction', icon: <Sparkles className="w-3.5 h-3.5 text-[#BE123C]" /> },
  ];
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#E2E8F0] shadow-xs transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          >
            <div className="w-10 h-10 rounded-2xl bg-[#1F2933] text-white flex items-center justify-center shadow-xs flex-shrink-0">
              <BookOpen className="w-5 h-5 text-[#2D6A4F]" />
            </div>
            <div className="flex-shrink-0">
              <div className="flex items-center gap-2">
                <span className="text-base font-extrabold tracking-tight text-[#0F172A] whitespace-nowrap">
                  Library Assistant
                </span>
          ...
          {currentUser?.role === 'student' && page !== 'student-hub' && (
            <div className="hidden md:flex items-center gap-1 bg-[#F8FAFC] p-1 rounded-2xl border border-[#E2E8F0] flex-shrink-0">
              <button
                ...
                className="px-2.5 py-1 text-xs font-semibold text-[#64748B] hover:text-[#0F172A] flex items-center gap-1 rounded-xl transition-colors cursor-pointer"
              >
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            {currentUser?.role === 'student' && onRequestPurchaseOpen && (
              <button
                ...
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-[#2D6A4F] hover:bg-[#22543D] text-white rounded-xl shadow-xs transition-all cursor-pointer whitespace-nowrap"
              >
            ...
            {currentUser?.role === 'student' && (
              <button
                ...
                className="relative p-2 text-[#475569] hover:text-[#0F172A] hover:bg-[#F1F5F9] rounded-xl transition-colors cursor-pointer"
            ...
            <div className="flex items-center gap-2 pl-2 border-l border-[#E2E8F0] flex-shrink-0">
              <div className="w-8 h-8 rounded-full bg-[#1F2933] text-white flex items-center justify-center text-xs font-bold ring-2 ring-[#E2E8F0] flex-shrink-0">
                {initials}
              </div>
              <div className="hidden sm:block text-left flex-shrink-0">
                <p className="text-xs font-bold text-[#0F172A] leading-tight whitespace-nowrap">
                  {currentUser?.name}
                </p>
