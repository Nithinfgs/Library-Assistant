    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#E2E8F0]">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#2D6A4F] hover:bg-[#22543D] text-white text-xs sm:text-sm font-bold rounded-xl shadow-md transition-all cursor-pointer"
        >
      ...
      <div className="flex items-center gap-2 bg-[#F1F5F9] p-1.5 rounded-2xl w-fit border border-[#E2E8F0] overflow-x-auto no-scrollbar">
        ...
        >
          <BookPlus className="w-4 h-4 text-[#4338CA]" />
        ...
        <div className="space-y-4">
          {activeLoans.length === 0 ? (
            <div className="bg-white rounded-3xl border border-[#E2E8F0] p-12 text-center text-[#64748B]">
              ...
              <h3 className="text-sm font-bold text-[#0F172A]">No Active Loans</h3>
              ...
              <button
                ...
                className="px-5 py-2.5 bg-[#2D6A4F] text-white text-xs font-bold rounded-xl shadow-md cursor-pointer"
              >
                Browse Books
              </button>
            </div>
                  ...
                  <div key={loan.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-[#94A3B8]">
                        Borrowed on {new Date(loan.borrowDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} • Due on <strong className="text-[#0F172A]">{new Date(loan.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</strong>
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button
                        ...
                        className="py-2 px-4 bg-[#EBF5EE] hover:bg-[#D4EDDA] text-[#2D6A4F] text-xs font-bold rounded-xl transition-colors cursor-pointer"
                      >
                        Return Book
                      </button>
                    </div>
        ...
        <div className="space-y-4">
          {myWaitlist.length === 0 ? (
            <div className="bg-white rounded-3xl border border-[#E2E8F0] p-12 text-center text-[#64748B]">
              ...
              <h3 className="text-sm font-bold text-[#0F172A]">No Active Waitlists</h3>
                ...
                <div key={entry.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    {entry.status === 'ready_for_pickup' && entry.pickupDeadline && (
                      <p className="text-xs font-bold text-[#2D6A4F] bg-[#EBF5EE] p-2 rounded-xl border border-[#2D6A4F]/20 mt-1">
        ...
        <div className="space-y-4">
          {myRequests.length === 0 ? (
            <div className="bg-white rounded-3xl border border-[#E2E8F0] p-12 text-center text-[#64748B]">
              ...
              <h3 className="text-sm font-bold text-[#0F172A]">No Book Requests</h3>
              <p className="text-xs text-[#94A3B8] max-w-sm mx-auto mt-1 mb-4">
              ...
              {myRequests.map((req) => {
                ...
                return (
                  <div key={req.id} className="p-5 space-y-2">
                    ...
                    {req.adminNotes && (
                      <p className="text-xs text-[#2D6A4F] bg-[#EBF5EE] p-2.5 rounded-xl border border-[#2D6A4F]/20">
        ...
        <div className="space-y-4">
          {returnedLoans.length === 0 ? (
            <div className="bg-white rounded-3xl border border-[#E2E8F0] p-12 text-center text-[#64748B]">
              ...
              <h3 className="text-sm font-bold text-[#0F172A]">No Returned Books Yet</h3>
              <p className="text-xs text-[#94A3B8]">
